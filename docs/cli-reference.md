...
| `--accepted-risks FILE` | file of the vendor's **intended / accepted-by-design behaviors** (threat model / known-limitations). Injected into audit + validate + corroborate so those behaviors are not reported as bugs. Additive, never inferred from the brief. Also available on `argo ingest`. |
